#!/usr/bin/env node
/**
 * Genera el PDF de Campaña localmente y lo publica en R2.
 *
 * Uso:
 *   npm run publish-campaign -- patch    # 0.0.0 → 0.0.1
 *   npm run publish-campaign -- minor    # 0.0.0 → 0.1.0
 *   npm run publish-campaign -- major    # 0.0.0 → 1.0.0
 *
 * Por defecto apunta a http://localhost:4200 (ng serve corriendo).
 * Para generar desde producción:
 *   APP_URL=https://firmware-wars.josepec.eu npm run publish-campaign -- patch
 */
...
// Secciones desde campaign.config.json, estilos PDF desde docs.config.json
const campaignCfgPath = join(ROOT, '..', 'firmware-wars', 'public', 'assets', 'config', 'campaign.config.json');
const pdfCfgPath = join(ROOT, '..', 'firmware-wars', 'public', 'assets', 'config', 'docs.config.json');
const cfgFull = JSON.parse(readFileSync(campaignCfgPath, 'utf-8'));
const cfg = JSON.parse(readFileSync(pdfCfgPath, 'utf-8')).pdf;
...
function getCurrentVersion() {
  const result = spawnSync(
    'npx',
    ['wrangler', 'kv', 'key', 'get', '--namespace-id', KV_NAMESPACE_ID, 'campaign-version', '--remote'],
...
const printUrl = `${appUrl}/docs/campaign-print?worker=1`;

const current = getCurrentVersion();
const next = bumpVersion(current, bump);
const version = ver(next);
const tmpFile = join(tmpdir(), `firmware-wars-campaign-v${version}.pdf`);

console.log(`\n📄 Generando PDF Campaña v${version}  (${ver(current)} → ${version})`);
...
try {
  ...
  const coverUrl = `${appUrl}/docs/cover-print?worker=1`
    + `&subtitle=${encodeURIComponent('Campaña')}`
  ...
  pdfDoc.getPages().forEach((p, i) => {
    ...
    const versionText = `CAMPAÑA v${version}`;
...
const r2Result = spawnSync(
  ...
  [
    ...
    `${R2_BUCKET}/campaign-v${version}.pdf`,
    '--file', tmpFile,
    '--remote',
  ],
  { cwd: ROOT, encoding: 'utf-8', shell: true },
...
await kvPut('campaign-version', next);

/* 4 — Limpiar */
unlinkSync(tmpFile);

console.log(`\n✅ PDF de Campaña publicado correctamente`);
console.log(`   Versión : v${version}`);
console.log(`   URL     : https://firmware-wars-api.josepec.eu/campaign-pdf\n`);
